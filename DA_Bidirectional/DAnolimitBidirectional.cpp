#include "DAnolimitBidirectional.h"

NodeForDAnolimitBidirectional::NodeForDAnolimitBidirectional(int capacity, int numkeys, NodeForDAnolimitBidirectional * parent) {
    NumOfKeys = numkeys;
    Parent = parent;
    Pointers = new NodeForDAnolimitBidirectional *[capacity];
...
DAnolimitBidirectional * NewDAnolimitBidirectional(int * array, int numItems, int degree ){
    DAnolimitBidirectional * da =new DAnolimitBidirectional(0, degree);
    if (array == nullptr) {
        da->Root = nullptr;
        return da;
    }
    ...
    da->Root->NumOfKeys = 1;
    da->Root->Pointers[0] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newa);
    return da;
}

DAnolimitBidirectional::DAnolimitBidirectional(int numItems, int capacity){
    NumItems = numItems;
    Capacity = capacity;
    Min = ceil(Capacity * 1.0 / 2.0);
    Root = new NodeForDAnolimitBidirectional(Capacity);
}


//return length of ans, ans is answer array
int* DAnolimitBidirectional::RangeQuery(int start, int end, int * lenOfAns) const {
    int dastart = 1;
    int daend = NumItems;
    int realstart = start;
    int realend = end;
    ...
    *lenOfAns = realnum;
    ...
    NodeForDAnolimitBidirectional* c = Root;
    int p = realstart;
    while (!c->IsLeaf) {
...
void DAnolimitBidirectional::PrintTree() {
    printf("\n");
    std::list<NodeForDAnolimitBidirectional *> lst;
    ...
    lst.push_back(Root);
    while (!lst.empty()) {
        NodeForDAnolimitBidirectional * ele = lst.front();
        if (ele != nullptr) {
            lst.pop_front();
            ...
            if (!ele->IsLeaf) {
                for (i = 0; i < ele->NumOfKeys; ++i) {
                    if (ele->Keys[i] != 0) {
                        NodeForDAnolimitBidirectional * c = ele->Pointers[i];
                        lst.push_back(c);
                    }
                }
            }
...
void DAnolimitBidirectional::PrintDA() {
    PrintTree();
    int preIndexInLeaf, preIndexInArray;
    NodeForDAnolimitBidirectional * leaf = findLeaf(1, &preIndexInLeaf, &preIndexInArray);
    while (leaf != nullptr) {
...
void DAnolimitBidirectional::PrintArray() const {
    int num;
...
int DAnolimitBidirectional::pathToRoot(NodeForDAnolimitBidirectional * child) const {
    int length = 0;
    NodeForDAnolimitBidirectional * c = child;
    while (c != Root) {
        c = c->Parent;
        length ++;
    }
...
NodeForDAnolimitBidirectional * makeLeafForDAnolimitBidirectional(int capacity) {
    NodeForDAnolimitBidirectional * n = new NodeForDAnolimitBidirectional(capacity);
    n->IsLeaf = true;
    return n;
}

int DAnolimitBidirectional::Depth() {
    if (Root == nullptr) {
        return 0;
    }
    int ans = 0;
...
void DAnolimitBidirectional::Insert(int newID, int pos) {
    NumItems ++;
    if (pos > NumItems + 1) {
        pos = NumItems + 1;
    }
    if (Root == nullptr) {
        Root = makeLeafForDAnolimitBidirectional(Capacity);
        Root->Keys[0] = 1;
        int * newa = new int[1];
        newa[0] = newID;
        Root->Pointers[0] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newa);
        Root->Parent = nullptr;
        Root->Next = nullptr;
    ...
    NodeForDAnolimitBidirectional * leaf;
    int preIndexInLeaf, preIndexInArray;
    ...
    if (preIndexInLeaf == -1) { // insert into the starting position
        ...
        NodeForDAnolimitBidirectional ** pointersToInsert = new NodeForDAnolimitBidirectional *[2];
        pointersToInsert[0] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newa);
        pointersToInsert[1] = reinterpret_cast<NodeForDAnolimitBidirectional *>(righta);
...
void DAnolimitBidirectional::insertMulKeysIntoLeaf(const int * keysToInsert, NodeForDAnolimitBidirectional** pointersToInsert,
                                      int numKeysToInsert, NodeForDAnolimitBidirectional* toLeaf, int toIndex) {
...
bool DAnolimitBidirectional::insertMulKeysIntoLeafAfterOneSplitting(const int * keysToInsert, NodeForDAnolimitBidirectional** pointersToInsert,
                                                       int numKeysToInsert, NodeForDAnolimitBidirectional* toLeaf, int toIndex) {
    int numGoRight = toLeaf->NumOfKeys - toIndex;
    int * keysGoRight = new int[numGoRight];
    NodeForDAnolimitBidirectional ** pointersGoRight = new NodeForDAnolimitBidirectional*[numGoRight];
    ...
    NodeForDAnolimitBidirectional * newLeaf = makeLeafForDAnolimitBidirectional(Capacity);
...
void DAnolimitBidirectional::insertOneIntoLeaf(NodeForDAnolimitBidirectional * leaf, int * newPointer, int length, int indexInLeaf) const {
    ...
    for (; i < leaf->NumOfKeys - 1; i++) {
        ...
        leaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(pointerToStore);
        keyToStore = tmpKey;
    ...
    NodeForDAnolimitBidirectional * c = leaf;
    NodeForDAnolimitBidirectional * parent = c->Parent;
...
void DAnolimitBidirectional::insertOneIntoLeafAfterSplitting( NodeForDAnolimitBidirectional * leaf, int * newPointer, int length, int indexInLeaf, bool needGoupwards) {
    NodeForDAnolimitBidirectional * newLeaf;
    int split=0, i=0, LeafNumPointers=0, NewLeafNumPointers=0;
    newLeaf = makeLeafForDAnolimitBidirectional(Capacity);
    split = Min;
    ...
    if (indexInLeaf < split) {// new array is in leaf (old leaf)
        ...
        leaf->Keys[i] = length;
        leaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newPointer);
        i--;
        for (; i >= 0; --i) {
            LeafNumPointers += leaf->Keys[i];
        }
        ...
        for (i = 0; i < newLeaf->NumOfKeys ; ++i) {
            if (i == indexInLeaf) {
                ...
                newLeaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newPointer);
                break;
            } else {
...
void DAnolimitBidirectional::insertIntoParent(NodeForDAnolimitBidirectional * left, int sumKeysLeft, NodeForDAnolimitBidirectional * right, int sumKeysRight, bool needGoupwards) {
    NodeForDAnolimitBidirectional * parent = left->Parent;
    if (parent == nullptr) {
...
void DAnolimitBidirectional::insertIntoNode(NodeForDAnolimitBidirectional * n, int leftIndex, int sumKeysRight, NodeForDAnolimitBidirectional * right, bool needGoUpward) {
    n->NumOfKeys ++;
    int i = 0;
    int keyToStore = sumKeysRight;
    NodeForDAnolimitBidirectional * pointerToStore = right;
    right->Parent = n;
    i = leftIndex + 1;
    for (; i < n->NumOfKeys - 1; i++) {
        int tmpKey = n->Keys[i];
        NodeForDAnolimitBidirectional * tmpPointer = n->Pointers[i];
        n->Keys[i] = keyToStore;
    ...
    int delta = 0;
    NodeForDAnolimitBidirectional * c = n;
    NodeForDAnolimitBidirectional * parent = c->Parent;
    if (parent == nullptr) {
        return;
    }
...
void DAnolimitBidirectional::insertIntoNodeAfterSplitting(NodeForDAnolimitBidirectional * parent, int leftIndex, int sumKeysRight, NodeForDAnolimitBidirectional * right, bool needGoUpwards) {
    int i, j, leftSumKeys = 0, rightSumKeys = 0;
    NodeForDAnolimitBidirectional * newNode;
    int * tempKeys;
    NodeForDAnolimitBidirectional** tempPointers;
    tempPointers = new NodeForDAnolimitBidirectional*[Capacity + 1];
    ...
    newNode = new NodeForDAnolimitBidirectional(Capacity);
    for ( i = 0; i < Min; ++i) {
...
int DAnolimitBidirectional::NodeSumKeys(NodeForDAnolimitBidirectional* n) {
    int ans = 0;
...
int DAnolimitBidirectional::getLeftIndex(NodeForDAnolimitBidirectional * parent, NodeForDAnolimitBidirectional * left) const {
    int leftIndex = 0;
...
void DAnolimitBidirectional::insertIntoNewRoot(NodeForDAnolimitBidirectional * left, int sumKeysLeft, NodeForDAnolimitBidirectional* right, int sumKeysRight) {
    Root = new NodeForDAnolimitBidirectional(Capacity);
...
NodeForDAnolimitBidirectional* DAnolimitBidirectional::findLeaf(int pos, int * preIndexInLeaf, int * preIndexInArray) const {
    if (pos > NumItems) {
        pos = NumItems;
    }
    bool flag = false;
...
void DAnolimitBidirectional::Delete(int pos) {
    if (pos > NumItems) {
        pos = NumItems;
    }
    int IndexInLeaf, IndexInArray;
    NodeForDAnolimitBidirectional * leaf = findLeaf(pos, &IndexInLeaf, &IndexInArray);
    ...
    if (oldlen == 1) { // delete single array
        ...
        leaf->Pointers[IndexInLeaf] = reinterpret_cast<NodeForDAnolimitBidirectional *>(&olda[1]);
...
void DAnolimitBidirectional::MergeOrRedistributeArrayAfterDelete(NodeForDAnolimitBidirectional * leaf, int IndexInLeaf, int IndexInArray) {
    if (IndexInLeaf == 0 && leaf->NumOfKeys == 1) { //root
        ...
        if (leaf->Keys[1] - 1 >= Capacity) { //redistribute
            ...
            leaf->Pointers[1] = reinterpret_cast<NodeForDAnolimitBidirectional *>(&(t[1]));
            leaf->Keys[1] --;
            SimplyAdjustAncestorKeysBy(leaf, -1);
            return;
            ...
            leaf->Keys[0] = newl;
            leaf->Pointers[0] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newa);
        ...
        if (leftlen - 1 >= Capacity) { //redistribute
            ...
            leaf->Pointers[IndexInLeaf] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newa);
...
void DAnolimitBidirectional::MergeOrRedistributeArrayAfterDeleteInMiddle(NodeForDAnolimitBidirectional * leaf, int IndexInLeaf, int IndexInArray) {
    if (leaf->Keys[IndexInLeaf+1] - 1 >= Capacity) { //redistribute with right
        ...
        leaf->Pointers[IndexInLeaf+1] = reinterpret_cast<NodeForDAnolimitBidirectional *>(&(t[1]));
        SimplyAdjustAncestorKeysBy(leaf, -1);
        return;
...
void DAnolimitBidirectional::insertOneIntoLeafForDeletion(NodeForDAnolimitBidirectional * leaf, int * newa, int length, int indexInLeaf) const {
    ...
    for (; i < leaf->NumOfKeys-1; i++) {
        tmpKey = leaf->Keys[i];
        NodeForDAnolimitBidirectional * tmpPointer = leaf->Pointers[i];
        leaf->Keys[i] = keyToStore;
        leaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(pointerToStore);
        keyToStore = tmpKey;
...
void DAnolimitBidirectional::deleteEntry(NodeForDAnolimitBidirectional * n, int indexInNode) {
    int neighbour_index = 0, capacity = 0;
    NodeForDAnolimitBidirectional * neighbour;
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::coalesceNodes(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
    int i, j;
    NodeForDAnolimitBidirectional * tmp;
...
int DAnolimitBidirectional::getLeftNeighborIndex(NodeForDAnolimitBidirectional * n) {
    int i = 0, nei = -1;
    NodeForDAnolimitBidirectional * parent = n->Parent;
...
void DAnolimitBidirectional::redistributeNodes(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
    ...
    if (neighbour_index != -1) { // left neighbour
        ...
        for (int j = numGoRight - 1; j >= 0 ; --j) {
            ...
            if (n->Pointers[j] != nullptr) {
                if (!n->IsLeaf) {
                    NodeForDAnolimitBidirectional * s = n->Pointers[j];
                    s->Parent = n;
                }
            }
            i--;
        ...
        for (i = 0; i <= indexToGoLeft; i++) {
            ...
            if (n->Pointers[tmp_j] != nullptr) {
                if (!n->IsLeaf) {
                    NodeForDAnolimitBidirectional * s = n->Pointers[tmp_j];
                    s->Parent = n;
                }
            }
            tmp_j++;
    ...
    NodeForDAnolimitBidirectional * c = n->Parent;
...
void DAnolimitBidirectional::adjustRoot() {
    NodeForDAnolimitBidirectional * newRoot = nullptr;
    if (Root->NumOfKeys > 1) {
        return;
    }
    if (!Root->IsLeaf) {
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::removeEntryFromNode(NodeForDAnolimitBidirectional * n, int indexInNode) {
    n->Keys[indexInNode] = 0;
    if (n->Pointers[indexInNode] != nullptr) {
        if (!n->IsLeaf) {
            NodeForDAnolimitBidirectional * deleted = n->Pointers[indexInNode];
...
void DAnolimitBidirectional::Reorder(int start, int end, int* neworder) const {
    if (end > NumItems) {
...
void DAnolimitBidirectional::coalesceNodesForDeleteOperation(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
...
void DAnolimitBidirectional::deleteEntryForDeleteOperatoin(NodeForDAnolimitBidirectional * n, int indexInNode) {
    ...
    if (nNumKeys >= min_keys) {
        NodeForDAnolimitBidirectional * tmp = n;
        while (tmp != Root) {
...
void DAnolimitBidirectional::redistributeNodesForDeleteOperation(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
    ...
    if (neighbour_index != -1) {
        ...
        NodeForDAnolimitBidirectional * temp_pointers[2*Capacity];
        int tmp_j = 0;
        ...
        for (i = 0; i < Capacity; i++) {
            ...
            if (!n->IsLeaf) {
                if (n->Pointers[i] != nullptr) {
                    NodeForDAnolimitBidirectional * s = n->Pointers[i];
                    s->Parent = n;
                }
            }
        }
...
void DAnolimitBidirectional::CutArray(int pos) {
    int IndexInLeaf, IndexInArray;
    NodeForDAnolimitBidirectional * leaf = findLeaf(pos, &IndexInLeaf, &IndexInArray);
    if (IndexInArray == 0) {
        return;
        ...
        if (leftl >= Capacity && rightl >= Capacity) {
            ...
            leaf->Pointers[IndexInLeaf] = reinterpret_cast<NodeForDAnolimitBidirectional *>(tmp);
            ...
            leaf->Keys[IndexInLeaf] = leftl;
            leaf->Pointers[IndexInLeaf] = reinterpret_cast<NodeForDAnolimitBidirectional *>(lefta);
            int * righta = &tmp[IndexInArray];
            if (leaf->NumOfKeys < Capacity) {
                insertOneIntoLeafForCut(leaf, righta, rightl, IndexInLeaf+1);
            } else {
...
void DAnolimitBidirectional::insertOneIntoLeafForCut(NodeForDAnolimitBidirectional *  leaf, int * newarray, int length, int indexInLeaf) {
...
void DAnolimitBidirectional::Swap(int start1, int end1, int start2, int end2) {
    ...
    NodeForDAnolimitBidirectional ** inter1 = nullptr;
    NodeForDAnolimitBidirectional ** inter2 = nullptr;
...
/*
void DAnolimitBidirectional::Swap(int start1, int end1, int start2, int end2, int* CutArrayTime, int* FindLeafTime, int* swapLeafTime, int*CheckMinTime) {
...
*/
int DAnolimitBidirectional::RangePosInLeafPointer(int start, int end, NodeForDAnolimitBidirectional *** inter, int * startIndexLeaf, int * endIndexLeaf){
    int numLeaf = 0;
    int numLeafMost = (end-start+1)/Min + 2;
    *inter = new NodeForDAnolimitBidirectional* [numLeafMost];
    int i_leafArray = 0;
    NodeForDAnolimitBidirectional * StartLeaf;
    int StartIndexInLeaf, StartIndexInArray;
    StartLeaf = findLeaf(start, &StartIndexInLeaf, &StartIndexInArray);
    NodeForDAnolimitBidirectional * c = StartLeaf;
    int length = end - start + 1;
...
/*
void DAnolimitBidirectional::CheckMinArrayAfterSwap(int s){
    if (s == 0) {
        return;
    }
    int IndexInLeaf, IndexInArray;
    NodeForDAnolimitBidirectional * leaf = findLeaf(s, &IndexInLeaf, &IndexInArray);
 ...
 */
/*
void DAnolimitBidirectional::MergeRedistributeArrayAfterSwap(NodeForDAnolimitBidirectional * leaf, int IndexInLeaf) {
    if (IndexInLeaf == 0) {
        ...
        if (rightl + leftl < Capacity) { // merge
            ...
            leaf->Keys[0]+= NumToMove;
            leaf->Pointers[1] = reinterpret_cast<NodeForDAnolimitBidirectional *>(&(righta[NumToMove])); // must be &()
            leaf->Keys[1]-=NumToMove;
            return;
        }
        ...
        if (leftl + rightl < Capacity) { // merge
            ...
            leaf->Keys[IndexInLeaf-1] -= NumToMove;
            leaf->Keys[IndexInLeaf] += NumToMove;
            leaf->Pointers[IndexInLeaf-1] = reinterpret_cast<NodeForDAnolimitBidirectional *>(lefta);
...
void DAnolimitBidirectional::deleteEntryForCheckMinArray(NodeForDAnolimitBidirectional * n, int indexInNode) {
    int min_keys, neighbour_index, capacity;
    NodeForDAnolimitBidirectional * neighbour;
    n = removeEntryFromNode(n, indexInNode);
    n->NumOfKeys--;
...
void DAnolimitBidirectional::redistributeNodesForCheckMinArray(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
    int i, leftNumKeys = 0, rightNumKeys = 0, leftIndex, leftSumKeys = 0, rightSumKeys = 0;
    ...
    if (neighbour_index != -1) { // neighbor (more keys) is on the left
        ...
        NodeForDAnolimitBidirectional ** temp_pointers = new NodeForDAnolimitBidirectional*[Capacity*2];
        int tmp_j = 0;
        for (i = indexToGoRight; i < neighbour->NumOfKeys; i++ ){
            temp_pointers[tmp_j] = neighbour->Pointers[i];
            temp_keys[tmp_j] = neighbour->Keys[i];
...
void DAnolimitBidirectional::coalesceNodesForCheckMinArray(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int neighbour_index) {
    int i, j, neighbour_insertion_index ;
    NodeForDAnolimitBidirectional *  tmp ;
    int n_index = neighbour_index + 1;
    //set neighbor to be the NodeForDAnolimitBidirectional on the left
    if (neighbour_index == -1) {
        tmp = n;
        n = neighbour;
        neighbour = tmp;
    ...
    if (neighbour->IsLeaf){
        neighbour->Next = n->Next;
        if (neighbour->Next != nullptr) {
            NodeForDAnolimitBidirectional * pNode = neighbour->Next;
...
int DAnolimitBidirectional::getIndexInParent(NodeForDAnolimitBidirectional * n) const {
    NodeForDAnolimitBidirectional * parent = n->Parent;
    for (int i = 0; i < Capacity; i++) {
        if (parent->Pointers[i] == n) {
            return i;
...
//--------------------------------------------------------- ***
void DAnolimitBidirectional::swapLeaf(NodeForDAnolimitBidirectional ** inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2, int startIndexLeaf1,
    ...
    NodeForDAnolimitBidirectional **node1 = inter1;
    NodeForDAnolimitBidirectional **node2 = inter2;
    //TimeVar time1 = timeNow();
    ...
    NodeForDAnolimitBidirectional * lastl1 = node1[numLeaf1-1];
    NodeForDAnolimitBidirectional * lastl2 = node2[numLeaf2-1];
...
void DAnolimitBidirectional::swapEndPointer(NodeForDAnolimitBidirectional* leftLeaf, int indexInLeftLeaf, int endIndexInLeftLeaf,
                               NodeForDAnolimitBidirectional * rightLeaf, int indexInRightLeaf , int endIndexInRightLeaf) {
    ...
    for (int k = 0; k < exchangeNum; k++) {
        NodeForDAnolimitBidirectional * tmpPointer = leftLeaf->Pointers[i];
...
void DAnolimitBidirectional::CallRedistributeMergeRecomputeKeysInAncestorsLeftPart(NodeForDAnolimitBidirectional ** leafToDelete, int numLeafToDelete) {
    NodeForDAnolimitBidirectional * leftLeafParent = leafToDelete[0]->Parent;
    NodeForDAnolimitBidirectional * rightLeafParent = leafToDelete[numLeafToDelete-1]->Parent;
...
void DAnolimitBidirectional::swapWholeLeafAndGoUpwards(NodeForDAnolimitBidirectional **wholeLeaf1, NodeForDAnolimitBidirectional **wholeLeaf2, int w1, int w2) {
    ...
    NodeForDAnolimitBidirectional *  LeftEdgeLeaf = nullptr, * RightEdgeLeaf = nullptr;
    NodeForDAnolimitBidirectional ** LeftParentArray = new NodeForDAnolimitBidirectional* [exchangeNum];
    NodeForDAnolimitBidirectional ** RightParentArray = new NodeForDAnolimitBidirectional* [exchangeNum];
    int lp = 0, rp = 0;
    NodeForDAnolimitBidirectional * lastLeftGrandParent = nullptr;
    NodeForDAnolimitBidirectional * lastRightGrandParent = nullptr;

    // adjust next, previous pointers
    NodeForDAnolimitBidirectional * LeftStartLeaf = wholeLeaf1[0];
    NodeForDAnolimitBidirectional * RightStartLeaf = wholeLeaf2[0];
    NodeForDAnolimitBidirectional * LeftEndLeaf = wholeLeaf1[exchangeNum-1];
    NodeForDAnolimitBidirectional * RightEndLeaf = wholeLeaf2[exchangeNum-1];
    if (LeftEndLeaf->Next == RightStartLeaf) {
        ...
        if (LeftEdgeLeaf->Next == RightEdgeLeaf) {
            NodeForDAnolimitBidirectional * LeftPre = LeftEdgeLeaf->Previous;
            ...
            NodeForDAnolimitBidirectional * LeftPre = LeftEdgeLeaf->Previous;
            NodeForDAnolimitBidirectional * RightPre = RightEdgeLeaf->Previous;
            RightEdgeLeaf->Previous = LeftPre;
            if (LeftPre != nullptr) {
                LeftPre->Next = RightEdgeLeaf;
            }
        ...
        NodeForDAnolimitBidirectional * LeftNext = LeftEdgeLeaf->Next;
        NodeForDAnolimitBidirectional * RightNext = RightEdgeLeaf->Next;
    ...
    for (int i = 0; i < exchangeNum; i++) {
        NodeForDAnolimitBidirectional * LeftLeaf = wholeLeaf1[i];
        NodeForDAnolimitBidirectional * RightLeaf = wholeLeaf2[i];
        NodeForDAnolimitBidirectional * LeftParent = LeftLeaf->Parent;
        NodeForDAnolimitBidirectional * RightParent = RightLeaf->Parent;
    ...
    if (remain != 0) {
        ...
        NodeForDAnolimitBidirectional **  leafToDelete;
        int numLeafToDelete ;
        ...
        // insert leaves
        if (remain == -1) { // left leaves more
            NodeForDAnolimitBidirectional * toParent = RightEdgeLeaf->Parent;
            ...
            NodeForDAnolimitBidirectional *  toParent = LeftEdgeLeaf->Parent;
...
void DAnolimitBidirectional::exchangeLeaf(NodeForDAnolimitBidirectional * LeftLeaf, NodeForDAnolimitBidirectional * LeftParent, NodeForDAnolimitBidirectional * RightLeaf, NodeForDAnolimitBidirectional * RightParent) const{
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::movePointerAfter(NodeForDAnolimitBidirectional * fromLeaf, int startIndex, int endIndex, NodeForDAnolimitBidirectional * toLeaf, int toIndex) {
    int * keysToInsert = new int[Capacity];
    NodeForDAnolimitBidirectional ** pointersToInsert = new NodeForDAnolimitBidirectional*[Capacity];
...
void DAnolimitBidirectional::insertLeavesIntoLeafParent(NodeForDAnolimitBidirectional * toParent, int startIndexInParent, NodeForDAnolimitBidirectional ** leafToInsert, int numLeafToInsert){
...
// no need to split
void DAnolimitBidirectional::insertMulLeavesIntoLeafParent(NodeForDAnolimitBidirectional * toParent, int startIndexInParent, NodeForDAnolimitBidirectional ** leafToInsert, int numLeafToInsert){
    NodeForDAnolimitBidirectional * last = leafToInsert[numLeafToInsert-1];
...
// adjust keys in ancestors of NodeForDAnolimitBidirectional c
void DAnolimitBidirectional::AdjustAncestorKeyForOneNode(NodeForDAnolimitBidirectional * c) {
    NodeForDAnolimitBidirectional * parent = c->Parent;
    int delta = -1;
    for (int i = 0; i < parent->NumOfKeys; i++){
        if (parent->Pointers[i] == c){
...
void DAnolimitBidirectional::insertMulLeavesIntoLeafParentSplitOnce(NodeForDAnolimitBidirectional * toParent, int startIndexInParent,
                                                       NodeForDAnolimitBidirectional ** leafToInsert, int numLeafToInsert) {
    int allNum = toParent->NumOfKeys + numLeafToInsert;
    int leftNum = allNum / 2;
    int rightNum = allNum - leftNum;
    int leftSum = 0, rightSum = 0;
    NodeForDAnolimitBidirectional ** allLeaf = new NodeForDAnolimitBidirectional*[Capacity + numLeafToInsert];
    ...
    NodeForDAnolimitBidirectional * newn = new NodeForDAnolimitBidirectional(Capacity);
...
void DAnolimitBidirectional::insertMulLeavesIntoLeafParentAfterSplitting(NodeForDAnolimitBidirectional * toParent, int startIndexInParent,
                                                            NodeForDAnolimitBidirectional ** leafToInsert, int numLeafToInsert) {
    //next, previous
    NodeForDAnolimitBidirectional * last = leafToInsert[numLeafToInsert-1];
    if (toParent->NumOfKeys == startIndexInParent) {
    ...
    NodeForDAnolimitBidirectional ** leafInToParentTail = new NodeForDAnolimitBidirectional*[numToParentAfterInsert];
    ...
    NodeForDAnolimitBidirectional ** parentNewSibling = new NodeForDAnolimitBidirectional*[parentNewSiblingNum];
    int LeafToInsertIdx = numToParentAfterInsert;
    int leafInToParentTailIdx = 0;
    for (int k = 0; k < parentNewSiblingNum; ++k) {
        parentNewSibling[k] = new NodeForDAnolimitBidirectional(Capacity);
...
void DAnolimitBidirectional::insertMulIntoParent(NodeForDAnolimitBidirectional * toParent, NodeForDAnolimitBidirectional ** parentNewSibling, int parentNewSiblingNum) {
    NodeForDAnolimitBidirectional * parent = toParent->Parent;
    if (parent == nullptr) {
...
void DAnolimitBidirectional::insertMulIntoNodeAfterSplitting(NodeForDAnolimitBidirectional * parent, int toIndex, NodeForDAnolimitBidirectional** pointerToInsert,
                                                int pointerToInsertNum) {
    NodeForDAnolimitBidirectional ** allPointers = new NodeForDAnolimitBidirectional*[Capacity + pointerToInsertNum];
    int j = 0;
...
void DAnolimitBidirectional::insertMulIntoNode(NodeForDAnolimitBidirectional * parent, int toIndex, NodeForDAnolimitBidirectional** parentNewSibling,
                                  int parentNewSiblingNum, int ParentIncrease) const {
    NodeForDAnolimitBidirectional ** tmpNodes = new NodeForDAnolimitBidirectional*[Capacity];
    int i = toIndex;
    int i_tmpNodes = 0;
    ...
    NodeForDAnolimitBidirectional * c = parent;
    parent = c->Parent;
    while (parent != nullptr ){
        for (i = 0; i < parent->NumOfKeys; i++) {
            if (parent->Pointers[i] == c) {
...
void DAnolimitBidirectional::insertMulIntoNewRoot(NodeForDAnolimitBidirectional * toParent, NodeForDAnolimitBidirectional ** parentNewSibling, int parentNewSiblingNum) {
    ...
    if (totalNum%Capacity == 0) { //all full nodes
        NodeForDAnolimitBidirectional * leftNode = new NodeForDAnolimitBidirectional(Capacity);
        int rightSiblingNum = numFullNodes - 1;
        NodeForDAnolimitBidirectional ** rightSibling = new NodeForDAnolimitBidirectional*[rightSiblingNum];
        int i_rightchild = 0;
        ...
        for (int i = 0; i < rightSiblingNum; i++) {
            rightSibling[i] = new NodeForDAnolimitBidirectional(Capacity);
        ...
        if (numFullNodes == 0) { //leftNodeForDAnolimitBidirectional is the second last one
            ...
            rightSibling[0] = new NodeForDAnolimitBidirectional(Capacity);
        ...
        } else { // leftNodeForDAnolimitBidirectional is not the second last one
            ...
            for (int k = 0; k < rightSiblingNum; k++) {
                rightSibling[k] = new NodeForDAnolimitBidirectional(Capacity);
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::movePointerBefore(NodeForDAnolimitBidirectional * fromLeaf, int startIndex, int endIndex, NodeForDAnolimitBidirectional * toLeaf, int toIndex) {
    int * keysToInsert = new int[Capacity];
    NodeForDAnolimitBidirectional ** pointersToInsert = new NodeForDAnolimitBidirectional*[Capacity];
    for (int i= startIndex; i <= endIndex; i++) {
        keysToInsert[i-startIndex] = fromLeaf->Keys[i];
...
/*
void DAnolimitBidirectional::PutLeavesIntoUpdateSumKeys(NodeForDAnolimitBidirectional ** leaves, int numLeaf) {
    int start = -1, end = 0;
    bool startpre = false, startprepre = false, endpost = false, endpostpost = false;
    NodeForDAnolimitBidirectional * first = nullptr, *last = nullptr, * p1 = nullptr,* p2 = nullptr,* n1 = nullptr, *n2 = nullptr;
    ...
    int newLIndex = 0;
    NodeForDAnolimitBidirectional ** newLeaves = new NodeForDAnolimitBidirectional * [newNum+5];
    if (startprepre) {
        newNum++;
...
void DAnolimitBidirectional::UpdateSumKeys(NodeForDAnolimitBidirectional ** nodes, int numNode) {
    if (nodes[0] == Root) {
        return;
    }
    if (numNode == 1) {
    ...
    NodeForDAnolimitBidirectional ** newArray = new NodeForDAnolimitBidirectional*[numNode];// make([]*Node, numNode)
    int numNewArray = 0;

    NodeForDAnolimitBidirectional * lastParent = nullptr, * parent = nullptr;
    int index = - 1, i = -1;
    for (i = 0; i < numNode; i ++) {
...
void DAnolimitBidirectional::AdjustAncestorKeysAccordingToCurNode(NodeForDAnolimitBidirectional * n) {
...
void DAnolimitBidirectional::SimplyAdjustAncestorKeysBy(NodeForDAnolimitBidirectional * node, int increase) const {
    if (node == Root) {
        return;
    }
    NodeForDAnolimitBidirectional * c = node;
    NodeForDAnolimitBidirectional * parent = c->Parent;
    while (parent != nullptr) {
        for (int i = 0; i < parent->NumOfKeys; ++i) {
            if (parent->Pointers[i] == c) {
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::CompleteAdjustAncestorNodesAfterDelete(NodeForDAnolimitBidirectional * n){
    if (n->NumOfKeys < Min) {
        NodeForDAnolimitBidirectional * re = nullptr;
        NodeForDAnolimitBidirectional * neighbour ;
...
void DAnolimitBidirectional::replaceKeysInLeaf(NodeForDAnolimitBidirectional * leaf, int startIdx, int endIdx, int * keysToInsert, NodeForDAnolimitBidirectional ** pointersToInsert, int numToInsert) {
...
void DAnolimitBidirectional::CheckAndMergeRedistributeNode(NodeForDAnolimitBidirectional *n) {
    if (n->NumOfKeys < Min) {
        NodeForDAnolimitBidirectional * neighbour ;
        int neighbour_index = getLeftNeighborIndex(n);
        if (neighbour_index == -1) {
...
//adjust from this NodeForDAnolimitBidirectional to root, and then redistribute/merge
//numKeysDeleted = -1 if omitted
NodeForDAnolimitBidirectional * DAnolimitBidirectional::adjustAfterDeleteKeysInNode(NodeForDAnolimitBidirectional * fromLeaf, int numKeysDeleted) {
    if (fromLeaf == Root) {
        adjustRoot();
        return nullptr;
    }
...
//only delete entries, no redistribution or merging
void DAnolimitBidirectional::deleteMulEntry(NodeForDAnolimitBidirectional ** entryToDelete, int numEntryToDelete){
    NodeForDAnolimitBidirectional ** newEntryToDelete = new NodeForDAnolimitBidirectional*[numEntryToDelete/Min+2];
    int i_newEntryToDelete = 0;
    NodeForDAnolimitBidirectional * lastParent = entryToDelete[0]->Parent;
    NodeForDAnolimitBidirectional * FirstParent = lastParent;
    int StartIndexFirstParent = getIndexInParent(entryToDelete[0]);
    NodeForDAnolimitBidirectional * curParent = nullptr;
    int OldNumKeysInCurParent = FirstParent->NumOfKeys;

    for (int i = 0; i < numEntryToDelete; i++) {
        NodeForDAnolimitBidirectional * curEntry = entryToDelete[i];
...
void DAnolimitBidirectional::movePointerInNodeForward(NodeForDAnolimitBidirectional * node, int startIndex, int endIndex, int toIndex){
...
void DAnolimitBidirectional::RedistributeMergeRecomputeKeysInAncestorsLeftPart(NodeForDAnolimitBidirectional * node) {
    if (node == nullptr) {
        return;
    }
    if (!node->IsLeaf) {
    ...
    if (node == Root) {
        if (node->NumOfKeys == 1) {
            NodeForDAnolimitBidirectional *newroot = node->Pointers[0];
            newroot->Parent = nullptr;
            Root = newroot;
        }
        return;
    ...
    NodeForDAnolimitBidirectional * parent = node->Parent;
...
void DAnolimitBidirectional::adjustAncestorKeysAfterExchangingLeaves(NodeForDAnolimitBidirectional ** nodeArray, int numNodeInArray) {
    if (nodeArray[0]->Parent == nullptr) {
        NodeForDAnolimitBidirectional * root = nodeArray[0];
        for (int i = 0; i < root->NumOfKeys; i++) {
            root->Keys[i] = NodeSumKeys(root->Pointers[i]);
        }
        return; //root
    }
    NodeForDAnolimitBidirectional ** newArray = new NodeForDAnolimitBidirectional *[numNodeInArray];
    int numNewArray = 0;
    NodeForDAnolimitBidirectional * lastParent = nullptr, * parent = nullptr;
    lastParent = nullptr;
    for (int i = 0; i < numNodeInArray; i++) {
        ...
        NodeForDAnolimitBidirectional * cur = nodeArray[i];
...
void DAnolimitBidirectional::swapHeadTail11(NodeForDAnolimitBidirectional * inter1, int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2,
                               int endIndexLeaf2) {

    int num = endIndexLeaf2 - startIndexLeaf1 + 1;
    int * Keys = new int[num];
    NodeForDAnolimitBidirectional ** Pointers = new NodeForDAnolimitBidirectional*[num];
    int ik = 0;
    ...
    NodeForDAnolimitBidirectional** nodes = new NodeForDAnolimitBidirectional*[1];
    nodes[0] = inter1;
...
void DAnolimitBidirectional::swapHeadTail12(NodeForDAnolimitBidirectional * inter1, NodeForDAnolimitBidirectional * inter2, int startIndexLeaf1, int endIndexLeaf1,
    ...
    NodeForDAnolimitBidirectional ** nodes = new NodeForDAnolimitBidirectional*[2];
    nodes[0] = inter1;
    nodes[1] = inter2;
...
void DAnolimitBidirectional::swapHeadTail21(NodeForDAnolimitBidirectional * inter1, NodeForDAnolimitBidirectional * inter2, int startIndexLeaf1, int endIndexLeaf1,
                               int startIndexLeaf2, int endIndexLeaf2) {
    int num =  endIndexLeaf2 + 1 + inter1->NumOfKeys-startIndexLeaf1;
    int * Keys = new int[num];
    NodeForDAnolimitBidirectional ** Pointers = new NodeForDAnolimitBidirectional*[num];
...
void DAnolimitBidirectional::swapHeadTail22(NodeForDAnolimitBidirectional * inter1, NodeForDAnolimitBidirectional * inter2, NodeForDAnolimitBidirectional * inter3, int startIndexLeaf1,
                               int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** nodes = new NodeForDAnolimitBidirectional*[3];
    nodes[0] = inter1;
    nodes[1] = inter2;
    nodes[2] = inter3;
...
void DAnolimitBidirectional::swapHeadTailLen1Is1(NodeForDAnolimitBidirectional **inter2, int numLeaf2, int startIndexLeaf1,
                                    int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    int NumToMove = startIndexLeaf2 - startIndexLeaf1;
    int* KeysToMove = new int[NumToMove];
    NodeForDAnolimitBidirectional ** Pointers = new NodeForDAnolimitBidirectional*[NumToMove];
    int ik = 0;
    for (int i = endIndexLeaf1 + 1; i < startIndexLeaf2; i++) {
...
void DAnolimitBidirectional::swapHeadTailLen2Is1(NodeForDAnolimitBidirectional **inter1, int numLeaf1, int startIndexLeaf1,
                                    int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    NodeForDAnolimitBidirectional * n = inter1[numLeaf1-1];
...
void DAnolimitBidirectional::swapHeadTailLen1Is2(NodeForDAnolimitBidirectional **inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                    int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    NodeForDAnolimitBidirectional * n = inter1[0];
    ...
    //insert
    NodeForDAnolimitBidirectional * y = inter2[numLeaf2-1];
...
void DAnolimitBidirectional::swapHeadTailLen2Is2(NodeForDAnolimitBidirectional **inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                    int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    NodeForDAnolimitBidirectional * n = inter2[0];
    ...
    //insert
    NodeForDAnolimitBidirectional * y = inter1[0];
...
void DAnolimitBidirectional::SwapRangesAdjacentWhenLensAreBoth2(NodeForDAnolimitBidirectional **inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                                   int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** nodes = new NodeForDAnolimitBidirectional*[4];
    nodes[0] = inter1[0];
    nodes[1] = inter1[1];
    nodes[2] = inter2[0];
    nodes[3] = inter2[1];
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::DeleteMidPartOfTwoAdjacentLeaves(NodeForDAnolimitBidirectional * leaf1, int startIndexInLeaf1, NodeForDAnolimitBidirectional * leaf2, int endIndexInLeaf2) {
    ...
    if (leaf1->NumOfKeys < Min) { // leaf1 < Min
        NodeForDAnolimitBidirectional * neighbour,* newl;
        ...
        if (neighbour_index == -1) { // leaf1 index is 0
            ...
            if (neighbour->NumOfKeys + leaf1->NumOfKeys <= Capacity) { // leaf1 and leaf2 coalesce
                ...
                if (newl->NumOfKeys < Min) {
                    ...
                    NodeForDAnolimitBidirectional * nei;
                    if (nei_index == -1) {
...
//to be chekced
void DAnolimitBidirectional::SwapRangesAdjacentRightIs2(NodeForDAnolimitBidirectional **inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                           int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
...
void DAnolimitBidirectional::SwapRangesAdjacentLeftIs2(NodeForDAnolimitBidirectional **inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                          int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    //delete
    NodeForDAnolimitBidirectional ** LeafToDelete = new NodeForDAnolimitBidirectional*[1];
...
void DAnolimitBidirectional::SWapRangesAdjacentWhenLensAreBoth1(NodeForDAnolimitBidirectional * leftnode, NodeForDAnolimitBidirectional * rightnode, int startIndexLeaf1,
                                                   int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** ns = new NodeForDAnolimitBidirectional*[2];
    ns[0] = leftnode;
    ns[1] = rightnode;
    UpdateSumKeys(ns, 2);
    delete []ns;
...
void DAnolimitBidirectional::SWapRangesAdjacentWhenLensAre1And2(NodeForDAnolimitBidirectional * leftnode, NodeForDAnolimitBidirectional* right1, NodeForDAnolimitBidirectional * right2,
                                                   int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** ns = new NodeForDAnolimitBidirectional*[3];
    ns[0] = leftnode;
    ns[1] = right1;
    ns[2] = right2;
    UpdateSumKeys(ns, 3);
...
void DAnolimitBidirectional::SwapRangesAdjacentWhenLensAre2And1(NodeForDAnolimitBidirectional * left1, NodeForDAnolimitBidirectional * left2, NodeForDAnolimitBidirectional * right,
                                                   int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
...
void DAnolimitBidirectional::SwapRangesAdjacentWhenLeftIs1(NodeForDAnolimitBidirectional * left, NodeForDAnolimitBidirectional ** inter2, int numLeaf2,
                                              int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
...
void DAnolimitBidirectional::SwapRangesAdjacentWhenRightIs1(NodeForDAnolimitBidirectional **inter1, int numLeaf1, NodeForDAnolimitBidirectional * right,
                                               int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
...
//to be ckeched
void DAnolimitBidirectional::SwapRangesAdjacent(NodeForDAnolimitBidirectional ** inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                   int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2){
    if (numLeaf1 == 1 && numLeaf2 == 1) {
    ...
    NodeForDAnolimitBidirectional** PointerToMove1 = new NodeForDAnolimitBidirectional*[numToMove1];
    int i1 = 0;
    for (int i = 0; i <= endIndexLeaf1; i++) {
    ...
    NodeForDAnolimitBidirectional** PointerToMove2 = new NodeForDAnolimitBidirectional*[numToMove2];
    int i2 = 0;
    ...
    NodeForDAnolimitBidirectional ** Middlepointers = new NodeForDAnolimitBidirectional*[NumMiddlePointers];
    int mi = 0;
    for (int i = endIndexLeaf1 + 1; i < inter1[numLeaf1-1]->NumOfKeys; i++) {
    ...
    inter2[0]->NumOfKeys=0;
    NodeForDAnolimitBidirectional ** ToDelete = new NodeForDAnolimitBidirectional*[2];
...
bool DAnolimitBidirectional::insertKeysIntoLeaf(int * keysToInsert, NodeForDAnolimitBidirectional ** pointersToInsert,
                                   int numKeysToInsert, NodeForDAnolimitBidirectional * toLeaf, int toIndex) {
...
void DAnolimitBidirectional::insertMulKeysIntoLeafAfterMulSplitting(const int * keysToInsert, NodeForDAnolimitBidirectional ** pointersToInsert,
                                                       int numKeysToInsert, NodeForDAnolimitBidirectional * toLeaf, int toIndex) {
    int totalNumKeys = numKeysToInsert + toLeaf->NumOfKeys;
    int * newKeys = new int[totalNumKeys];
    NodeForDAnolimitBidirectional ** newPointers = new NodeForDAnolimitBidirectional*[totalNumKeys];
    int i = 0;
    ...
    if (remainNumLeaf == 0 || remainNumLeaf >= Min) { // full nodes + remainnumleaf
        ...
        NodeForDAnolimitBidirectional ** parentNewSibling = new NodeForDAnolimitBidirectional*[parentNewSiblingNum];
        for (int k = 0; k < parentNewSiblingNum; k++) {
            parentNewSibling[k] = makeLeafForDAnolimitBidirectional(Capacity);
            for (int y = 0; y < Capacity; y++) {
        ...
        NodeForDAnolimitBidirectional * pa = toLeaf->Parent;
        ...
        NodeForDAnolimitBidirectional ** paNode = &parentNewSibling[0];
        NodeForDAnolimitBidirectional * pa = toLeaf->Parent;
        int startindex = getIndexInParent(toLeaf)+1;
        pa->Keys[startindex-1] = NodeSumKeys(toLeaf);
...
void DAnolimitBidirectional::SwapWhenLeftLenIs2(NodeForDAnolimitBidirectional * left1, NodeForDAnolimitBidirectional * left2, int startIndexLeaf1, int endIndexLeaf1,
                                   NodeForDAnolimitBidirectional ** inter2, int numLeaf2, int startIndexLeaf2, int endIndexLeaf2) {
    //delete leaves
    ...
    //insert leaves
    NodeForDAnolimitBidirectional * toParent = left1->Parent;
    ...
    NodeForDAnolimitBidirectional ** pointersToInsert20 = new NodeForDAnolimitBidirectional*[numToInsert20];
    ...
    NodeForDAnolimitBidirectional ** pointersToInsert22 = new NodeForDAnolimitBidirectional*[numToInsert22];
    ...
    NodeForDAnolimitBidirectional ** pointersToMoveInLeft = new NodeForDAnolimitBidirectional*[numToMoveInLeft];
...
void DAnolimitBidirectional::ReplaceMidOfTwoAdjacentLeaves(NodeForDAnolimitBidirectional * left, NodeForDAnolimitBidirectional * right, int startIndexLeft, int endIndexRight,
                                              int * keysToInsert, NodeForDAnolimitBidirectional ** pointersToInsert, int numToInsert) {
...
void DAnolimitBidirectional::SwapWhenBothLensAre2(NodeForDAnolimitBidirectional * left1, NodeForDAnolimitBidirectional * left2, int startIndexLeaf1, int endIndexLeaf1,
                                     NodeForDAnolimitBidirectional * right1, NodeForDAnolimitBidirectional * right2, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    if (len1 < len21) {
        ...
        NodeForDAnolimitBidirectional ** pointers1 = new NodeForDAnolimitBidirectional*[len1];
        int ikeys1 = 0;
        for (int i = startIndexLeaf1; i < left1->NumOfKeys; i++) {
        ...
        for (int i = startIndexLeaf2; i < right1->NumOfKeys; i++) {
            ...
            keys1[i2] = tmp;
            NodeForDAnolimitBidirectional * tmpp = right1->Pointers[i];
        ...
        NodeForDAnolimitBidirectional ** pointersToMove = new NodeForDAnolimitBidirectional*[numKeysToMove];
        int ik = 0;
        ...
        for (int i = 0; i <= endIndexLeaf2; i++) {
            ...
            NodeForDAnolimitBidirectional* tmpp = right2->Pointers[i];
...
void DAnolimitBidirectional::SwapWhenBothLensAre1(NodeForDAnolimitBidirectional * leaf1, int startIndex1, int endIndex1, NodeForDAnolimitBidirectional * leaf2,
    ...
    for (int i = 0; i < exchangeNum; i++) {
        ...
        NodeForDAnolimitBidirectional * tmpp = leaf1->Pointers[startIndex1+i];
    ...
    if (numToMove1 == numToMove2) {
        ...
        NodeForDAnolimitBidirectional ** pointers = new NodeForDAnolimitBidirectional*[numKeys];
...
void DAnolimitBidirectional::SwapWhenLensAre1And2(NodeForDAnolimitBidirectional * leaf1, int startIndex1, int endIndex1, NodeForDAnolimitBidirectional * leaf2,
                                     int startIndex2, NodeForDAnolimitBidirectional * leaf3, int endIndex3) {
    ...
    if (len1 < len2) {
        for (int i = 0; i < len1; i++) {
            ...
            NodeForDAnolimitBidirectional * tmpp = leaf2->Pointers[startIndex2+i];
            leaf2->Pointers[startIndex2+i] = leaf1->Pointers[startIndex1+i];
            leaf1->Pointers[startIndex1+i] = tmpp;
        }
        int numToMove = len2 - len1 + len3;
        int * keysToMove = new int[numToMove];
        NodeForDAnolimitBidirectional ** pointersToMove = new NodeForDAnolimitBidirectional*[numToMove];
        int tk = 0;
        ...
        NodeForDAnolimitBidirectional ** pointersToMove = new NodeForDAnolimitBidirectional*[len3];
        for (int i = 0; i < len3; i++) {
        ...
        for (int i = 0; i < newStartIn3; i++) {
            ...
            leaf1->Keys[indexIn1] = tmpk;
            NodeForDAnolimitBidirectional* tmpp = leaf3->Pointers[i];
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::coalesceNodesStopAtParent(NodeForDAnolimitBidirectional * n, NodeForDAnolimitBidirectional * neighbour, int  neighbour_index) {
    int i, j, neighbour_insertion_index;
    NodeForDAnolimitBidirectional * tmp;
    int n_index = neighbour_index + 1;
    //set neighbor to be the NodeForDAnolimitBidirectional on the left
    ...
    if (neighbour->IsLeaf) {
        neighbour->Next = n->Next;
        if (neighbour->Next != nullptr) {
            NodeForDAnolimitBidirectional * tmp = neighbour->Next;
...
NodeForDAnolimitBidirectional* DAnolimitBidirectional::swapStartPointer(NodeForDAnolimitBidirectional * leftLeaf, int endIndexInLeftLeaf, NodeForDAnolimitBidirectional *rightLeaf, int endIndexInRightLeaf) {
    int i = 0;
    int j = 0;
...
void DAnolimitBidirectional::SwapForTailHead(NodeForDAnolimitBidirectional ** inter1, NodeForDAnolimitBidirectional ** inter2, int numLeaf1, int numLeaf2,
                                int startIndexLeaf1, int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** midPointers = new NodeForDAnolimitBidirectional*[numMidKeys];
    int midIdx = 0;
    for (int i = 0; i <= endIndexLeaf2; ++i) {
    ...
    NodeForDAnolimitBidirectional ** pointersBegin2 = new NodeForDAnolimitBidirectional*[numBegin2];
    ...
    NodeForDAnolimitBidirectional ** pointersEnd1 = new NodeForDAnolimitBidirectional*[numEnd1];
...
void DAnolimitBidirectional::SwapWhenLenIs1(NodeForDAnolimitBidirectional * LoneLeaf, NodeForDAnolimitBidirectional **inter2, int numLeaf2, int startIndexLeaf1,
                               int endIndexLeaf1, int startIndexLeaf2, int endIndexLeaf2) {
    ...
    NodeForDAnolimitBidirectional ** pointersToInsertAfterFullLeaves = new NodeForDAnolimitBidirectional*[numToInsertAfterFullLeaves];// make([]interface{}, NumInRightPart);
    for (int i = 0; i <= endIndexLeaf2; ++i) {
    ...
    NodeForDAnolimitBidirectional ** poitnersToMoveInLoneLeaf = new NodeForDAnolimitBidirectional*[numToMoveInLoneLeaf];
    ...
    NodeForDAnolimitBidirectional ** poitnersToMoveInBeginInter2 = new NodeForDAnolimitBidirectional*[numToMoveInBeginInter2];
    ...
    NodeForDAnolimitBidirectional ** leafToInsert = &inter2[1];
    int numLeafToDelete = numLeaf2 - 2;
    //delete leaves
    NodeForDAnolimitBidirectional * firstl = leafToInsert[0];
    NodeForDAnolimitBidirectional * lastl = leafToInsert[numLeafToDelete-1];
    ...
    //insert leaves
    NodeForDAnolimitBidirectional * toParent = LoneLeaf->Parent;
...
void DAnolimitBidirectional::Move(int start_, int end_, int des_) {
    ...
    //PrintTree();
    int toindex = -1;
    NodeForDAnolimitBidirectional * toleaf = CutForDesInMove(des, &toindex);
...
void DAnolimitBidirectional::RealMove(int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional ** inter1, int numLeaf1, NodeForDAnolimitBidirectional * toleaf, int toindex) {
    if (numLeaf1 == 1) {
        ...
        if (endIndexLeaf1== inter1[numLeaf1-1]->NumOfKeys-1) {
            NodeForDAnolimitBidirectional ** leaves = new NodeForDAnolimitBidirectional*[1];
            leaves[0] = toleaf;
            SwapRangesAdjacent(inter1, leaves, numLeaf1, 1, startIndexLeaf1, endIndexLeaf1, 0, toindex-1);
            delete []leaves;
        } else {
            NodeForDAnolimitBidirectional ** leaves = new NodeForDAnolimitBidirectional*[2];
...
void DAnolimitBidirectional::MoveToLeafIsFirstLeaf(NodeForDAnolimitBidirectional ** inter1, int numLeaf1, int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional * toleaf, int toindex) {
    if (numLeaf1 == 1) {
...
void DAnolimitBidirectional::MoveToLeafIsLastLeaf(NodeForDAnolimitBidirectional ** inter1, int numLeaf1, int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional * toleaf, int toindex) {
    if (numLeaf1==1) {
...
void DAnolimitBidirectional::Move1(NodeForDAnolimitBidirectional ** inter1, int numLeaf1, int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional * toleaf, int toindex) {
    //NodeForDAnolimitBidirectional ** leafToMove = new NodeForDAnolimitBidirectional* [numLeaf1-2];
    int NumLeafToMove = numLeaf1-2;
    NodeForDAnolimitBidirectional ** leafToMove = &inter1[1]; // only move whole leaves

    NodeForDAnolimitBidirectional * fl = leafToMove[0]->Previous;
    if (fl != nullptr) {
    ...
    int startIndex = getIndexInParent(toleaf)+1;
    NodeForDAnolimitBidirectional * toParent = toleaf->Parent;
    ...
    NodeForDAnolimitBidirectional ** PointersBefore = new NodeForDAnolimitBidirectional*[numKeysBefore];
    ...
    NodeForDAnolimitBidirectional ** PointersAfter = new NodeForDAnolimitBidirectional*[numKeysAfter];
    int afterIdx = 0;
...
void DAnolimitBidirectional::MoveWhenLenIs1(NodeForDAnolimitBidirectional ** inter1, int numLeaf1, int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional * toleaf, int toIndex) {
    NodeForDAnolimitBidirectional * fromLeaf = inter1[0];
...
void DAnolimitBidirectional::MoveWhenLenIs2(NodeForDAnolimitBidirectional ** inter1, int numLeaf1, int startIndexLeaf1, int endIndexLeaf1, NodeForDAnolimitBidirectional * toleaf, int toIndex) {
    NodeForDAnolimitBidirectional * fromLeaf1 = inter1[0];
    NodeForDAnolimitBidirectional * fromLeaf2 = inter1[1];
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::CutForDesInMove(int des, int* toIndex) {
    int IndexInLeaf, IndexInArray;
    NodeForDAnolimitBidirectional * leaf = findLeaf(des, &IndexInLeaf, &IndexInArray);
    if (des > NumItems) {
...
NodeForDAnolimitBidirectional * DAnolimitBidirectional::insertOneIntoLeafAfterSplittingForMove(NodeForDAnolimitBidirectional * leaf, int * newID, int length, int indexInLeaf, int * toIndex) {
    NodeForDAnolimitBidirectional * new_leaf;
    int split= Min, i, newIndex = -1;
    int LeafNumPointers = 0, NewLeafNumPointers = 0;
    new_leaf = makeLeafForDAnolimitBidirectional(Capacity);
    ...
    if (indexInLeaf < split) { // new array is in leaf (old leaf)
        ...
        LeafNumPointers += length;
        leaf->Keys[i] = length;
        leaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newID);
        newIndex = i;
        i--;
        for (; i >= 0; i--) {
            LeafNumPointers += leaf->Keys[i];
        ...
        *toIndex = newIndex;
        ...
        for (i = 0; i < new_leaf->NumOfKeys; i++) {
            if (i == indexInLeaf) {
                ...
                new_leaf->Pointers[i] = reinterpret_cast<NodeForDAnolimitBidirectional *>(newID);
                newIndex = i;
                break;
            } else {
