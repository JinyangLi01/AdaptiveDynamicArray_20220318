...
#define DELETE_SAMPLE "DELETE FROM insert1M WHERE ID = ?"
...
/* Bind the buffers */
    ...
    for (int lt = 0; lt < operations; lt++) {
      ...
      int * Ansll = ll->RangeQuery(1, NowTotalNum, &tmpnum);
      if (! CompareArray(Ansda, Anssa, tmpnum)) {
          flag1 = true;
      }
      if (! CompareArray(Ansll, Anssa, tmpnum)) {
          flag2 = true;
      }
      if (! CompareArray(Ansll, Ansda, tmpnum)) {
          flag3 = true;
      }
       ...
       */
            ...
            //printf("here is printing, lt = %d, before insert\n", lt);
