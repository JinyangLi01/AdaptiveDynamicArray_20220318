...
#define DELETE_SAMPLE "DELETE FROM query100 WHERE ID = ?"
...
int main() {
    ...
    queryStmt = mysql_stmt_init(mysql);
    if (!(insertStmt && deleteStmt && queryStmt))
    {
    ...
    queryString += ")";
    char QUERY_SAMPLE[queryString.length() + 1];
    strcpy(QUERY_SAMPLE, queryString.c_str());

    if (mysql_stmt_prepare(queryStmt, QUERY_SAMPLE, strlen(QUERY_SAMPLE)))
    {
        fprintf(stderr, " mysql_stmt_prepare(), QUERY failed\n");
        fprintf(stderr, " %s\n", mysql_stmt_error(queryStmt));
        exit(0);
    }
    fprintf(stdout, " prepare, INSERT DELETE QUERY successful\n");
/* Get the parameter count from the statement */
    ...
    param_count_query = mysql_stmt_param_count(queryStmt);
    fprintf(stdout, " total parameters in QUERY: %d\n", param_count_query);
    if (param_count_query != 5000) /* validate parameter count */
    {
        fprintf(stderr, " invalid query parameter count returned by MySQL\n");
        exit(0);
    }
/* Bind the data for all 3 parameters */
    memset(bindInsert, 0, sizeof(bindInsert));
    memset(bindDelete, 0, sizeof(bindDelete));
    memset(bindQuery, 0, sizeof(bindQuery));
...
/* STRING PARAM */
    ...
    for (int k = 0; k < 5000; ++k) {
        bindQuery[0].buffer_type= MYSQL_TYPE_LONG;
        bindQuery[0].buffer= (char *)&queryID[k];
        bindQuery[0].is_null= 0;
        bindQuery[0].length= 0;
    }
...
/* Bind the buffers */
    ...
    if (mysql_stmt_bind_param(queryStmt, bindQuery))
    {
        fprintf(stderr, " mysql_stmt_bind_param() failed\n");
        fprintf(stderr, " %s\n", mysql_stmt_error(queryStmt));
        exit(0);
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        if ( (lt+1 <= 10) || ((lt+1<=100) && ((lt+1)%10 == 0)) || ((lt+1<=1000) && ((lt+1)%100 == 0))
            ...
            if (mysql_stmt_execute(queryStmt))
            {
    ...
    finstant.close();
    /* Close the statement */
    if ( mysql_stmt_close(insertStmt) || mysql_stmt_close(deleteStmt) || mysql_stmt_close(queryStmt) )
    {
