...
int main() {
    ...
    string filepath[3] = {"../Experiments/AveBaseline/100K/3_instant.csv", "../Experiments/AveBaseline/100K/3_average.csv", "../Experiments/AveBaseline/100K/3_final.csv"};
    ...
    int InsertActions = operations * 5 / 100;
    int DeleteActions = operations * 5 / 100;
    int QueryActions = operations * 75 / 100;
    int ReorderActions = operations * 5 / 100;
    int SwapActions = operations * 5 / 100;
    int MoveActions = operations * 5 / 100;
    ...
    CountedBtree * cbt = NewTreeForArray(CbtOrder, iniNum);
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        switch (a[lt]) {
            case 1: //query
            {
                ...
                time1 = timeNow();
                int *AnsCBT = cbt->RangeQuery(start, end, &NumCBT);
                time2 = timeNow();
                int tcbt = duration(time2 - time1);
                Tcbt[1] += tcbt;
                Tcbt[0] += tcbt;
                Incbt = tcbt;
        ...
        if ((lt+1)%ComputeInterval == 0) {
            ...
            finstant<<fl<<" ,"<<Insda<<","<<Inssa<<","<<Incbt<<","<<Insll<<endl;
        }
    ...
    faverage<<"Counted Btree,"<< Tcbt[1]/ShortQueryActions<<","
            << Tcbt[7]/LongQueryActions<<"," << Tcbt[2]/InsertActions<<","
            << Tcbt[3]/DeleteActions<<","<<Tcbt[4]/ReorderActions<<","
            << Tcbt[5]/SwapActions<<","<<Tcbt[6]/SwapActions<<endl;
