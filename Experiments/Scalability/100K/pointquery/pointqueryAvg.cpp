...
int main(int argc, char** argv) {
    string filepath[3] = {"pointqueryAvg.csv", "pointqueryAvgLog.txt"};
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        if ((lt+1)%10000 == 0)
        {
            ...
            for (int k = 0; k < repeatNum; ++k) {
                int start1 = RandomInt(1, NowTotalNum-1);
                int num;
                int qans;

                time1 = timeNow();
                qans = da->Query(start1);
                time2 = timeNow();
                t1 += duration(time2-time1);

                time1 = timeNow();
                qans = sa->Query(start1);
                time2 = timeNow();
                t2 += duration(time2-time1);

                time1 = timeNow();
                qans = ll->Query(start1);
                time2 = timeNow();
