...
int main(int argc, char** argv) {
    string filepath[3] = {"query500Avg.csv", "query500AvgLog.txt"};
    ...
    int queryLength = 500, queryNum = 0;
    int iniNum = 1000;
    int danodesize = 10;
    int CbtOrder = 10;
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        if ((lt+1)%10000 == 0)
        {
            ...
            int start1 = RandomInt(1, NowTotalNum-queryLength-3);
            int end1 = start1 + queryLength -1;
            int num;
            int * ans;
            ...
            for (int k = 0; k < repeatNum; ++k) {
                time1 = timeNow();
                ans = da->RangeQuery(start1, end1, &num);
                time2 = timeNow();
                ...
                time1 = timeNow();
                ans = ll->RangeQuery(start1, end1, &num);
                time2 = timeNow();
                t3 = duration(time2 - time1);
                Tll += t3;

                time1 = timeNow();
                ans = cbt->RangeQuery(start1, end1, &num);
                time2 = timeNow();
                t4 = duration(time2 - time1);
                Tcbt += t4;

                time1 = timeNow();
                ans = tiered.RangeQuery(start1, end1, num);
                time2 = timeNow();
    ...
    finstant <<Tda/queryNum << ","<< Tsa/queryNum<<","<<Tll/queryNum<<","<<Tcbt/queryNum<<","<<Ttv/queryNum<<endl;
