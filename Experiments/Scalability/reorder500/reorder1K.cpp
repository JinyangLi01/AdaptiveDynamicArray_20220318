...
int main() {
    ...
    string filepath[3] = {"reorder1K.csv", "reorder1KLog.txt"};
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        if ((lt+1)%10000 == 0)
        {
            ...
            int start1 = RandomInt(1, NowTotalNum - reorderLength - 2);
            int end1 = start1+ reorderLength-1;

            int numOldArray;
            int * oldArray = new int[reorderLength];
            for (int j = 0; j < reorderLength; ++j) {
                oldArray[j] = standard_array[start1+j];
            }
            // sa->RangeQuery(start, end, &NumSA);
            int * newArray = new int[reorderLength];
            for (int j = 0; j < reorderLength; ++j) {
                newArray[j] = oldArray[reorderLength-j-1];
            }
            ...
            time2 = timeNow();
            t5 = duration(time2 - time1);
            Ttv += t5;

            time1 = timeNow();
            for (int i = start1-1; i <= end1-1; ++i) {
                vec[i] = newArray[i-start1+1];
            }
            time2 = timeNow();
            t6 = duration(time2 - time1);
            Tvec += t6;


            finstant <<t1 << ","<< t2<<","<<t3<<","<<t4<<","<<t5<<","<<t6<<endl;
    ...
    finstant <<Tda/reorderNum << ","<< Tsa/reorderNum<<","<<Tll/reorderNum<<","
    <<Tcbt/reorderNum<<","<< Ttv/reorderNum <<"," << Tvec/reorderNum <<endl;
