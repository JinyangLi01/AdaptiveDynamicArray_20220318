...
int * VectorRangeQuery(vector<int> &vec, int start, int end, ofstream & flog) {
...
int * SARangeQuery(int * & array, int start, int end, ofstream & flog) {
    int queryLength = end - start + 1;
    int * qans = new int[queryLength];
    TimeVar time1, time2;
    int time_sum = 0;
...
//            flog<<"da depth = "<<da->Depth() << endl;
        ...
        if ( lt % 2 == 0 ) {
            ...
            time1 = timeNow();
            qans = SARangeQuery(standard_array, start1, end1, flog);
            time2 = timeNow();
            ...
            time1 = timeNow();
            qans = VectorRangeQuery(vec, start1, end1, flog);
            time2 = timeNow();
