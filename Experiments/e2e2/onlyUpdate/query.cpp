...
int main() {
    ...
    try {
        ...
        pstmtInsert = con->prepareStatement("INSERT INTO delete100 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        pstmtDelete = con->prepareStatement("DELETE FROM delete100 WHERE ID = ?");
        string queryString = "SELECT AVG(Age) FROM query100 WHERE ID in (?";
        ...
        finstant << " ,DA,SA,LL,CBT,DBtime" << endl;
        ...
        for (int lt = 0; lt < TotalActions; lt++) {
            ...
            if ((lt + 1 <= 10) || ((lt + 1 <= 100) && ((lt + 1) % 10 == 0)) ||
                ...
                Tll += s;
                Tcbt += s;
                finstant << Tda << "," << Tsa << "," << Tll << "," << Tcbt << "," << s << endl;
