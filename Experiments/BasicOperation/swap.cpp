...
int main() {
    ...
    string filepath[3] = {"swap.csv", "swapLog.txt"};
    ...
    Seq::Tiered<int, LayerItr<LayerEnd, Layer<30, Layer<30, Layer<30>>>>> tiered;
    ...
    for (int lt = 0; lt < operations; lt++) {
        ...
        if ( (lt+1 <= 10) || ((lt+1<=100) && ((lt+1)%10 == 0)) || ((lt+1<=1000) && ((lt+1)%100 == 0))
            ...
            if (!CompareArray(Ansst, Anstv, num)) {
                flag3 =true;
            }
            ...
            delete []Ansst, delete []Ansdanl, delete []AnsDA, delete []Anstv;


            numUpdate ++;

            long long Tda = 0, Tdanl = 0, Tst = 0, Ttv = 0, Tsa = 0;
            CurOutputNum ++;
           ...
           // int start1 = 30, end1 = 10000, start2 = 45000, end2 = 82000;
            ...
            finstant <<Tda << ","<< Tdanl<<","<<Tst<<","<<Tsa<<","<<Ttv<<endl;
