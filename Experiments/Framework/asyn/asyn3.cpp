/*
 * this file is different from asyn only that
 * 1. always sleep 500 ms
 * about 5.5 hours
 * error:
 * lt = 81000da depth = 3
lt = 81500da depth = 3
...
Aborted
 It always aborted at 81500
 I guess whether it is because the number of unjoined threads is too large
 so I make asyn5 which detach threads in insert/delete within the lt loop
...
int main() {
    ...
    try {
        ...
        for (int l = 0; l < numSqlCons; ++l) {
            ...
            sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO asyn3 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM asyn3 WHERE ID = ?");
        ...
        string filepath[3] = {"asyn3.csv", "asyn3Log.txt"};
        ...
        for (int lt = 0; lt < operations; lt++) {
            ...
            if ((lt + 1 <= 10) || ((lt + 1 <= 100) && ((lt + 1) % 10 == 0)) ||
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(500));
                finstant << Tinsert[TinsertIdx] << ","<<Tinsert[TinsertIdx]+TinsertDB[TinsertIdx];
                cout << Tinsert[TinsertIdx] << ","<<Tinsert[TinsertIdx]+TinsertDB[TinsertIdx];
                TinsertIdx++;
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(500));
                finstant << ","<<Tdelete[TdeleteIdx] << "," << Tdelete[TdeleteIdx] + TdeleteDB[TdeleteIdx] << endl;
                cout <<","<<Tdelete[TdeleteIdx] << "," << Tdelete[TdeleteIdx] + TdeleteDB[TdeleteIdx] << endl;
                TdeleteIdx++;
                numUpdate += 2;
            }
        } //for lt <= loopTime
