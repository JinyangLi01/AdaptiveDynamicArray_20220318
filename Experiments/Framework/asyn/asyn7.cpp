...
int main() {
    ...
    try {
        ...
        for (int l = 0; l < numSqlCons; ++l) {
            ...
            sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO asyn7 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM asyn7 WHERE ID = ?");
        ...
        string filepath[3] = {"asyn7.csv", "asyn7Log.txt"};
