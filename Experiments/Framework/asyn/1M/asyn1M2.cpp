/*
 * This is uses 1M dataset, 100K operations
 * always sleep 500 ms
 * about ?? hours
 * danodesize = 100
...
int main() {
    ...
    try {
        ...
        for (int l = 0; l < numSqlCons; ++l) {
            ...
            sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO asyn1M2 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM asyn1M2 WHERE ID = ?");
        ...
        string filepath[3] = {"asyn1M2.csv", "asyn1M2Log.txt"};
