/*
 * this file is to test insert/delete to db in thread
 * this is executable no error reported
 */
#include <iostream>
#include <fstream>
#include <chrono>
...
void insertToDB(int ToInsert, int insertindex, int threadIdx) {
    ...
    if (!find) { // build a new connection
        ...
        sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO updateInThread VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM updateInThread WHERE ID = ?");
...
int main() {
    ...
    try {
        ...
        string filepath[3] = {"updateInThread.csv", "updateInThreadLog.txt"};
        ...
        for (int lt = 0; lt < operations; lt++) {
            ...
            time1 = timeNow();
            int pos = 3;// RandomInt(1, NowTotalNum);
