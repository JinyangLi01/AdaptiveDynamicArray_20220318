/*
 * 1000 operations, sleep for 500ms after each scrolling
 */
#include <iostream>
#include <fstream>
#include <chrono>
...
int main() {
    ...
    try {
        ...
        sql::PreparedStatement* pstmtInsert = con->prepareStatement("INSERT INTO prefetching5 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        sql::PreparedStatement* pstmtDelete = con->prepareStatement("DELETE FROM prefetching5 WHERE ID = ?");

        string queryString = "SELECT * FROM prefetching5 WHERE ID in (?";
        ...
        string filepath[3] = {"prefetching5.csv", "prefetching5Log.txt"};
