/*
 * 1,000,000 operations, iniNum = 1000000, sleep for 500ms after each scrolling
 */
#include <iostream>
#include <fstream>
#include <chrono>
...
int main() {
    ...
    try {
        ...
        sql::Connection* con = driver->connect("localhost", "root", "ljy19980228");
        /* Connect to the MySQL test database */
        con->setSchema("end2end");


        sql::PreparedStatement* pstmtInsert = con->prepareStatement("INSERT INTO prefetching1M2 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        sql::PreparedStatement* pstmtDelete = con->prepareStatement("DELETE FROM prefetching1M2 WHERE ID = ?");

        string queryString = "SELECT * FROM prefetching1M2 WHERE ID in (?";
        ...
        for (int l = 0; l < numSqlCons; ++l) {
            ...
            sqlconn->sqlconnection = driver->connect("localhost", "root", "ljy19980228");
            /* Connect to the MySQL test database */
        ...
        string filepath[3] = {"prefetching1M2.csv", "prefetching1M2Log.txt"};
