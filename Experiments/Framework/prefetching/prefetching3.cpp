/*
 * 10,000 operations, sleep for 500ms after each scrolling
 */
#include <iostream>
#include <fstream>
#include <chrono>
...
int main() {
    ...
    try {
        ...
        sql::PreparedStatement* pstmtInsert = con->prepareStatement("INSERT INTO prefetching3 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        sql::PreparedStatement* pstmtDelete = con->prepareStatement("DELETE FROM prefetching3 WHERE ID = ?");

        string queryString = "SELECT * FROM prefetching3 WHERE ID in (?";
        ...
        string filepath[3] = {"prefetching3.csv", "prefetching3Log.txt"};
