...
void insertToDB(int ToInsert, int insertindex, int threadIdx) {
    ...
    if (!find) { // build a new connection
        ...
        sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO prefetching VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM prefetching WHERE ID = ?");
