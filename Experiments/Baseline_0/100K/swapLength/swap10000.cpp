/*
 * distance between two ranges is random
 * in 2-0706-swapLength distance is fixed
 */
#include <iostream>
#include <fstream>
#include <chrono>
...
int main(int argc, char** argv) {
    string filepath[3] = {"swap10000.csv", "swap10000Log.txt"};
    ...
    int swapLength = 10000;// atoi(argv[3]);
    finstant<<"swapLength,DA,SA,LL,CBT,TV"<<endl;
...
/*
        ...
        if ( (lt+1)%10000 == 0 ) {
            ...
            while (!(start1 < start2 && end1 < start2 && end2 < NowTotalNum)) {
                int x = RandomInt(1, NowTotalNum);
                int y = RandomInt(1, NowTotalNum);
                if (x < y) {
                    start1 = x;
                    start2 = y;
                } else {
    ...
    finstant <<swapLength<<"," <<Tda/swapNum << ","<< Tsa/swapNum <<","<<Tll/swapNum<<","<<Tcbt/swapNum<<","<<Ttv/swapNum<<endl;
