...
int main() {
    ...
    string filepath[3] = {"2-0702-query50.csv", "2-0702-query50Log.txt"};
    ...
    for (int lt = 0; lt < operations; lt++) {
        ...
        if ( (lt+1 <= 10) || ((lt+1<=100) && ((lt+1)%10 == 0)) || ((lt+1<=1000) && ((lt+1)%100 == 0))
            ...
            finstant << fl << ",";
            int length = 50, start, end;
            int num, loop = 50;
            int * qans;
            for (int i = 0; i < loop; ++i) {
                start = RandomInt(1, NowTotalNum-length-1);
                end = start + length - 1;
                time1 = timeNow();
                qans = sa->RangeQuery(start, end, &num);
                time2 = timeNow();
                Tsa += duration(time2 - time1);

                time1 = timeNow();
                qans = da->RangeQuery(start, end, &num);
                time2 = timeNow();
                Tda += duration(time2 - time1);

                time1 = timeNow();
                qans = ll->RangeQuery(start, end, &num);
                time2 = timeNow();
                Tll += duration(time2 - time1);

                time1 = timeNow();
                qans = cbt->RangeQuery(start, end, &num);
                time2 = timeNow();
                Tcbt += duration(time2 - time1);

                time1 = timeNow();
                qans = tiered.RangeQuery(start, end, num);
                time2 = timeNow();
                Ttv += duration(time2 - time1);
            }
            finstant <<Tda/loop << ","<< Tsa/loop<<","<<Tll/loop<<","<<Tcbt/loop<<","<<Ttv/loop<<endl;
