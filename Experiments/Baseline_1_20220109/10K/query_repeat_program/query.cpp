...
//    filepath[0] = "insert_instant.csv";
//    filepath[1] = "insert_log.txt";
//    ofstream finstant, flog, ffinal;
//    finstant.open(filepath[0], ios::out | ios::in | ios::trunc);
//    flog.open(filepath[1], ios::out | ios::in | ios::trunc);
    ...
    for (int lt = 0; lt < TotalActions; lt++) {
        ...
        int num;
        int * list_sa = sa->RangeQuery(1, NowTotalNum, &num);
        int * list_vec = VectorRangeQuery(vec, 1, NowTotalNum);

        if (! CompareArray(list_sa,list_vec, NowTotalNum))
        {
            throw std::invalid_argument("SA != vector");
        }
