...
const int iniNum = 225800; // movie dataset 110k awards
...
void DoQueries(int num_queries, int pos) {
    ...
    for (int numPre = 0; numPre < num_queries; ++numPre) {
        ...
        if (ready) {
            if (numPre % 2 == 0) {
                pool.enqueue(
                    QueryDB0, rows1, IDs, num, TqueryIdx, threadIdx
                );
                ...
                pool.enqueue(
                    QueryDB1, rows1, IDs, num, TqueryIdx, threadIdx
                );
...
void DoDelete(int num_delete) {
    ...
    for (int i = 0; i < num_delete; i ++) {
        ...
        time2 = timeNow();
        pool.enqueue(
            deleteFromDB, delete_id, 1, threadIdx
        );
