...
const int iniNum = 220000; // movie dataset 110k awards
...
void insertToDB(int ToInsert, int insertindex, int threadIdx) {
    ...
    int Year_insert = RandomInt(1800, 2021);
    insertStmt->setInt(4, Age_insert);
    for (int k = 5; k <= 7; ++k) {
        int thisLen = RandomInt(1, STRING_SIZE);
        insertStmt->setString(k, RandomString(thisLen));
    }
...
int main() {
    ...
    try {
        ...
        for (int l = 0; l < numSqlCons; ++l) {
            ...
            sqlconn->pstmtInsert = sqlconn->sqlconnection->prepareStatement("INSERT INTO awards VALUES (?, ?, ?,  ?, ?, ?, ?)");
            sqlconn->pstmtDelete = sqlconn->sqlconnection->prepareStatement("DELETE FROM awards WHERE id = ?");
        ...
        string filepath[3] = {"wl1.csv", "wl1Log.txt"};
